  ...
  <div class="scan-out-container">
    ...
    <div class="batch-list-section">
      <var-card shadow="hover">
        <h2 class="section-title">{{ $t('outboundRecords.filter.inProgress') }}</h2>
        <!-- 移动端卡片布局 -->
            ...
            <div class="mobile-batch-item">
              ...
              <div class="mobile-batch-actions">
                <var-button
                  type="primary"
                  size="small"
                  @click="goToScanStep(batch.serialNumber)"
                  style="width: 100%"
                >
                ...
                <var-button
                  type="success"
                  size="small"
                  @click="shipBatch(batch.serialNumber)"
    ...
    <var-popup
      ...
      :title="$t('outboundRecords.addBatch')"
      position="center"
      width="90%"
      @close="resetForm"
    >
      ...
      <div class="step-content">
        ...
        <div v-if="activeStep === 0" class="step-item">
          <var-form ref="formRef" label-position="top">
            <var-form-item :label="$t('scanOut.step1.serialNumber')">
              <var-input
                v-model="outboundForm.batchNumber"
                :placeholder="$t('scanOut.step1.serialNumber')"
            ...
            <div class="step-buttons">
               <var-button type="primary" @click="nextStep" :disabled="!outboundForm.batchNumber">{{ $t('common.next') }}</var-button>
            </div>
          </var-form>
        </div>
        ...
        <div v-if="activeStep === 1" class="step-item">
          <var-form ref="formRef" label-position="top">
            ...
            <var-form-item :label="$t('scanOut.step2.channel')">
              <var-select
                v-model="outboundForm.channel"
                :placeholder="$t('scanOut.step2.selectChannel')"
                size="normal"
            ...
            <div class="step-buttons">
              ...
              <var-button type="primary" @click="nextStep" :disabled="!outboundForm.channel">{{ $t('common.confirm') }}</var-button>
            </div>
          </var-form>
        </div>
      </div>
